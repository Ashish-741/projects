...
const joSHA = require('jssha');
const { v4: uuid } = require('uuid');
const { isLoggedIn } = require('../middleware'); // We are using request for making an HTTP/HTTPS call to payumoney server
const request = require('request');

router.post('/payment_gateway/payumoney', isLoggedIn, (req, res) => {
    req.body.txnid = // Here pass txnid and it should be different on every call
    req.body.email = req.user.email;
    req.body.firstname = req.user.firstname; // Here save all the details in pay object 
    const pay = req.body;
    ...
    sha.update(hashString); // Getting hashed value from sha module
